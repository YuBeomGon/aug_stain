...
def get_svd(image) :
    U, sigma, V = torch.linalg.svd(image.T, full_matrices=False)
    return U, sigma
...
#         self.bias = nn.Parameter(torch.tensor([0., 0., 0,]), requires_grad=True)
        ...
        self.linear_evaluation = linear_evaluation
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.linear_evaluation == True and self.training == True :
            B, C, H, W = x.shape
            features = torch.split(x, B)
            f_list = []
            for f in features[0] :
                
                stain_mat, _ = get_svd(torch.permute(f, (1,2,0)).reshape(-1, C))
                conv_img = torch.matmul(torch.permute(f, (1,2,0)), stain_mat)
                for i in range(C):
                    alpha = torch.tensor(np.random.uniform(1-.2, 1+.2))
                    conv_img[:,:,i] *= alpha
                inv_img = torch.matmul(conv_img, stain_mat.T)
                f_list.append(torch.permute(inv_img, (2,0,1)))
            aug_features = torch.stack(f_list,  dim=0)
            # print('new', aug_features.shape)
            x = aug_features.detach()
...
def resnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    return _resnet("resnet18", BasicBlock, [2, 2, 2, 2, 2], pretrained, progress, **kwargs)
